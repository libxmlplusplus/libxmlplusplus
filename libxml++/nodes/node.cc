...
std::optional<ustring> Node::get_name2() const
{
  if (!impl_->name)
    return {};
...
std::optional<ustring> Node::get_path2() const
{
  xmlChar* path = xmlGetNodePath(impl_);
  if (!path)
    return {};
  std::optional<ustring> result = (char*)path;
  xmlFree(path);
  return result;
}
...
std::optional<ustring> Node::get_namespace_prefix2() const
{
...
std::optional<ustring> Node::get_namespace_uri2() const
{
  if(impl_->type == XML_DOCUMENT_NODE ||
     impl_->type == XML_HTML_DOCUMENT_NODE ||
     impl_->type == XML_ENTITY_DECL ||
